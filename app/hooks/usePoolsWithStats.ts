...
export function usePoolsWithStats({
  ...
  const poolStatsQueries = useSuiClientQueries({
  ...
  const pairStatsQueries = useQueries({
    queries: (poolsData ?? []).map((pool) => ({
      ...
      queryFn: () =>
        fetchPairStatsWithRewards(pool.poolId, range, sinceForDynamo),
  ...
  const processedPools = useMemo(() => {
    if (
      ...
      !poolStatsQueries.data ||
      poolStatsQueries.isError ||
      poolStatsQueries.isPending
    ) {
      return null;
    }
    ...
    const poolsWithStats: PoolWithStats[] = poolsData.map((pool, index) => {
      ...
      const poolStats = poolStatsQueries.data[index];
      const raw = (poolStats?.data?.content as any)?.fields as any;
      return {
        ...pool,
        rewardsDistributed: stats?.rewardsDistributed
          ? Number(stats.rewardsDistributed)
          : 0,
        balanceA: raw?.balance_a ?? 0,
        balanceB: raw?.balance_b ?? 0,
        burnFee: raw?.burn_fee ?? 0,
        creatorRoyaltyFee: raw?.creator_royalty_fee ?? 0,
        creatorRoyaltyWallet: raw?.creator_royalty_wallet ?? "",
        lockedLpBalance: raw?.locked_lp_balance ?? 0,
        lpBuilderFee: raw?.lp_builder_fee ?? 0,
        rewardBalance: raw?.reward_balance_a ?? 0,
        rewardsFee: raw?.rewards_fee ?? 0,
      };
    }).filter((pool) => pool.balanceA > 0 && pool.balanceB > 0);
    ...
    poolsWithStats.forEach((pool) => {
      if (pool.coinB.symbol === "SRM") {
        ...
        featuredCoinBSymbol &&
        pool.coinB.symbol === featuredCoinBSymbol
      ) {
        featuredPool = pool;
      } else if (
        pool.poolId ===
        "0xf6f85ac36b35edac1016beb9c67ec7051126fc24677b55c2892cea1585726a76"
      ) {
        // do nothing
      } else {
  ...
  return {
    ...
    error:
      poolsError ||
      (pairStatsQueries.isError ? pairStatsQueries.errors[0] : null),
  };
}

const fetchPairStatsWithRewards = async (
  poolId?: string,
  range?: string,
  since?: number
) => {
